...
struct ray_t
{
	...
	vector3d_aligned  start;			// starting point, centered within the extents
	vector3d_aligned  delta;			// direction + length of the ray
	vector3d_aligned  start_offset;	// Add this to m_Start to get the actual ray start
	vector3d_aligned  extents;		// Describes an axis aligned box extruded along a ray
	...
	void init(vector3d const& start, vector3d const& end)
	{
		...
		assert( &end );
		vector3d_subtract(end, start, delta);

		is_swept = (delta.length_sqr() != 0);

		vector3d_clear(extents);
		is_ray = true;

		vector3d_clear(start_offset);
		vector3d_copy(start, this->start);
	...
	void init(vector3d const& start, vector3d const& end, vector3d const& mins, vector3d const& maxs)
	{
		...
		vector3d_subtract(maxs, mins, extents);
		extents *= 0.5f;
		is_ray = (extents.length_sqr() < 1e-6);

		vector3d_add(mins, maxs, start_offset);
		start_offset *= 0.5f;

		vector3d_add(start, start_offset, this->start);
		start_offset *= -1.0f;
